...
import com.rokid.movie.taobao.api.internal.util.LogUtils;
...
public class AutoRetryTaobaoClient extends DefaultTaobaoClient {
    private static String LOG_TAG = AutoRetryTaobaoClient.class.getSimpleName();
        ...
        for (int i = 0; i <= maxRetryCount; i++) {
            if (i > 0) {
                if ((rsp != null && ((rsp.getSubCode() != null && rsp.getSubCode().startsWith("isp."))
                    ...
                    LogUtils.w(LOG_TAG, buildRetryLog(request.getApiMethodName(), request.getTextParams(), i));
                } else {
                    break;
                }
            }
