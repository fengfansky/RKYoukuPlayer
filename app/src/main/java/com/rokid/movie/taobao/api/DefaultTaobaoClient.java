package com.rokid.movie.taobao.api;

import android.util.Log;

import com.rokid.movie.taobao.api.internal.parser.json.ObjectJsonParser;
import com.rokid.movie.taobao.api.internal.parser.xml.ObjectXmlParser;
import com.rokid.movie.taobao.api.internal.util.RequestParametersHolder;
import com.rokid.movie.taobao.api.internal.util.TaobaoHashMap;
import com.rokid.movie.taobao.api.internal.util.TaobaoLogger;
import com.rokid.movie.taobao.api.internal.util.TaobaoUtils;
import com.rokid.movie.taobao.api.internal.util.WebUtils;
...
public class DefaultTaobaoClient implements TaobaoClient {
    ...
    private String topOneCode = null;
    ...
    private <T extends TaobaoResponse> RequestParametersHolder doPost(TaobaoRequest<T> request, String session)
            throws ApiException {
        ...
        if (targetAppKey != null)
            protocalOptParams.put(TARGETAPPKEK, targetAppKey);
        if (topOneCode != null)
            protocalOptParams.put(TOPONECODE, topOneCode);
        ...
        try {
            ...
            if (request instanceof TaobaoUploadRequest) {
                ...
                rsp = this.doPost(reqUrl.toString(), appParams, fileParams, Constants.CHARSET_UTF8, connectTimeout,
                        readTimeout, request.getHeaderMap());
            } else {
                rsp = this.doPost(reqUrl.toString(), appParams, Constants.CHARSET_UTF8, connectTimeout, readTimeout,
                        request.getHeaderMap());
            }
            Log.e("http", "result = " + rsp);
    ...
    @Override
    public <T extends TaobaoResponse> void executeAsync(TaobaoRequest<T> request, String session,
                                                        TaobaoCallback<T> callback) {
        TaobaoParser<T> parser = null;
        if (this.needEnableParser) {
            if (Constants.FORMAT_XML.equals(this.format)) {
                parser = new ObjectXmlParser<T>(request.getResponseClass());
        ...
        _executeAsync(request, parser, session, callback);

    }

    @Override
    public <T extends TaobaoResponse> void executeAsync(TaobaoRequest<T> request, TaobaoCallback<T> callback) {
        TaobaoParser<T> parser = null;
        if (this.needEnableParser) {
            if (Constants.FORMAT_XML.equals(this.format)) {
                parser = new ObjectXmlParser<T>(request.getResponseClass());
        ...
        _executeAsync(request, parser, null, callback);

    }

    private <T extends TaobaoResponse> void _executeAsync(TaobaoRequest<T> request, TaobaoParser<T> parser,
                                                          String session, TaobaoCallback<T> callback) {
        if (this.needCheckRequest) {
            try {
                request.check();
            } catch (ApiRuleException e) {
                ...
                try {
                    ...
                    callback.onError(localResponse, e.getErrMsg());
                }
                localResponse.setErrorCode(e.getErrCode());
                localResponse.setMsg(e.getErrMsg());
                callback.onError(localResponse, e.getErrMsg());
        ...
        doPostAsync(request, parser, session, callback);

        return;
    }

    private <T extends TaobaoResponse> void doPostAsync(TaobaoRequest<T> request, TaobaoParser<T> parser,
                                                        String session, TaobaoCallback<T> callback) {
        RequestParametersHolder requestHolder = new RequestParametersHolder();
        TaobaoHashMap appParams = new TaobaoHashMap(request.getTextParams());
        requestHolder.setApplicationParams(appParams);
        ...
        try {
            ...
            WebUtils.doPostAsync(reqUrl.toString(), appParams, Constants.CHARSET_UTF8, connectTimeout, readTimeout,
                    request.getHeaderMap(), parser, requestHolder, callback);
    ...
    protected String doPost(String url, Map<String, String> params, String charset, int connectTimeout,
                            int readTimeout, Map<String, String> headerMap) throws Exception {
        return WebUtils.doPost(url, params, charset, connectTimeout, readTimeout, headerMap);
    }

    protected String doPost(String url, Map<String, String> params, Map<String, FileItem> fileParams, String charset,
